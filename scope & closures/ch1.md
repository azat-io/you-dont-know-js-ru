...
Одним из ключевых механизмов языков программирования является сохранение значений в переменных, а позже извлечение или изменение этих значений. Это основной метод, которым мы пользуемся при моделировании *состояния* программы.

Без такого концепта программы могли бы выполнять лишь весьма ограниченный и не очень интересный круг задач.

Но, чтобы понять, как переменные работают, сначала следует спросить: где переменные *живут*? Другими словами, где они хранятся? И, что более важно, как наша программа их находит, когда нуждается в них?

Эти вопросы говорят о необходимости хорошо определенного набора правил для управления переменными. Мы назовем этот набор правил — *Область видимости*.

*Области видимости* полностью определяются на этапе компиляции.

## Теория компиляторов

Это может быть очевидно или удивительно, в зависимости от вашего опыта с различными языками, но несмотря на то, что JavaScript подпадает под категорию "динамических" или "интерпретируемых" языков, на самом деле он является компилируемым языком. Он *не* компилируется заранее, как многие традиционно компилируемые языки, и результаты компиляции не являются переносимыми среди различных распределенных систем.

Тем не менее, шаги, которые предпринимает среда исполнения JavaScript, схожи с теми, которые делают традиционные компиляторы.

В общем случае, обработка программы компилятором происходит в три этапа:
...
1. **Разбиение на лексемы (Tokenizing/Lexing)**: разбиение строки символов на имеющие смысл (для языка) части, называемые лексемами. Например, представьте программу: `var a = 2;`. Эта программа, вполне вероятно, будет разбита на следующие лексемы: `var`, `a`, `=`, `2` и `;`. Пробел может быть сохранен или не сохранен, как лексема, в зависимости от того, имеет он смысл или нет.
    ...
    **Примечание:** Разница между *tokenizing* и *lexing* едва уловима, но она определяется тем, идентифицируются ли лексемы *с учётом контекста* или нет. Проще говоря, если токенизатор в процессе парсинга использует текущий контекст для выяснения следует ли считать `a` отдельной лексемой или только частью другой лексемы, *тогда* это будет **lexing**.

2. **Парсинг:** берет поток (массив) лексем и превращает его в дерево вложенных элементов, которые вместе формируют грамматическую структуру программы. Это дерево называется "AST" (<b>A</b>bstract <b>S</b>yntax <b>T</b>ree, дерево абстрактного синтаксиса).
...
3. **Генерация кода:** процесс превращения AST в исполняемый код. Эта часть сильно зависит от языка, платформы назначения и т.п..

    Вместо того, чтобы увязать в деталях, мы просто опустим их и скажем, что есть способ взять наше вышеописанное AST для `var a = 2;` и превратить его в набор машинных инструкций, чтобы в действительности *создать* переменную с именем `a` (включая выделение памяти и т.д.), а затем сохранить значение в `a`.

    **Примечание:** подробности того, как движок управляет системными ресурсами, глубже, чем мы будем "копать", поэтому примем на веру, что движок умеет создавать и сохранять переменные, когда необходимо.

Компиляция JavaScript включает гораздо больше, чем эти три шага. Например, в процессе парсинга и генерации кода безусловно есть шаги по оптимизации быстродействия выполнения, включая удаление лишних элементов.

Поэтому здесь я лишь очерчиваю границы, но вы скоро увидите, почему эти детали важны, несмотря на то, что мы рассматриваем их на довольно высоком уровне.

Для начала, движки JavaScript не могут позволить себе тратить массу времени на оптимизацию, в отличие от других компиляторов, так как JavaScript *не* компилируется заранее.

Для JavaScript, компиляция во многих случаях происходит за микросекунды (или меньше!) перед выполнением кода. Чтобы гарантировать высочайшее быстродействие, движки JS используют все виды уловок (такие как JIT, который компилирует лениво и даже перекомпилирует на ходу), которые вне области нашего обсуждения.
...
Чтобы лучше понять процесс анализа переменных, его удобно представить в виде различных диалогов, происходящих внутри движка.

Встречайте действующих лиц:
...
3. *Область видимости*: еще один друг *Движка*; собирает и обслуживает список поиска всех объявленных идентификаторов (переменных), и устанавливает строгий набор правил доступа к этим идентификатором для кода, выполняемого в текущий момент.

Для *полного понимания* как работает JavaScript, вам необходимо начать *думать*, как *Движок* (и его друзья), задавать их вопросы, и отвечать так же, как они.

### Туда и обратно

Когда вы видите программу `var a = 2;`, вы вероятнее всего думаете о ней, как об одной команде. Но наш новый друг *Движок* видит две отдельные команды, одну, из которых *Компилятор* обработает на этапе компиляции, а вторую обработает *Движок* во время выполнения.

Первая вещь, которую сделает *Компилятор* с этой программой, это разбиение на лексемы, которые он затем распарсит в дерево.

Хочется предположить, что код, который будет сгенерирован на основании этого дерева, можно описать так: "Выделить память для переменной, пометить ее как `a`, затем поместить в неё значение `2`." К сожалению, это не совсем точно.

*Компилятор* вместо этого сделает следующее:

1. Встретив `var a`, *Компилятор* просит *Область видимости* посмотреть существует ли уже переменная `a` в коллекции текущей области видимости. Если да, то *Компилятор* игнорирует это объявление переменной и двигается дальше. В противном случае, *Компилятор* просит *Область видимости* объявить новую переменную `a` в коллекции текущей области видимости.

2. Затем *Компилятор* генерирует код, чтобы обработать присваивание `a = 2`. Этот код сначала спрашивает *Область видимости*, есть ли переменная с именем `a`, доступная в коллекции текущей области видимости. Если да, то *Движок* использует эту переменную. Если нет, то *Движок* ищет *в другом месте* (см. раздел *Вложенная область видимости* ниже).
